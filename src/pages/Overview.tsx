...
export default function Overview(_props: Props) {
  // Note: v1 Day‑1 flow is intentionally self‑contained.
  // We can connect it to persistent storage in a later phase.
  ...
  const applyAlternative = () => {
    // Alternative intentionally keeps current status.
    setMode("confirm");
  };
  ...
  const actionTitle = action === "cancel" ? "Cancel trip" : action === "change" ? "Change trip" : "Something went wrong";
  const actionText =
    action === "cancel"
      ...
      <section className="to-section">
        <div className="to-card" style={{ padding: 14, borderRadius: 14 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            <div>
              ...
              <div className="to-muted" style={{ marginTop: 4 }}>Test Mode — No real bookings or payments</div>
            </div>
          ...
          <div className="to-card" style={{ padding: 16, borderRadius: 14, marginTop: 14 }}>
            ...
            <div className="to-grid" style={{ display: "grid", gridTemplateColumns: "1fr", gap: 12, maxWidth: 520 }}>
              <div>
                ...
                <input className="to-input" value={bookingRef} onChange={(e) => setBookingRef(e.target.value)} placeholder="e.g., ABC123" />
              </div>
              <div>
                <div className="to-k">Airline or hotel</div>
                <input className="to-input" value={vendor} onChange={(e) => setVendor(e.target.value)} placeholder="e.g., Delta / Marriott" />
              </div>
              <div>
        ...
        {mode === "overview" ? (
          <div className="to-card" style={{ padding: 16, borderRadius: 14, marginTop: 14 }}>
            ...
            <div className="to-muted">
              Reference: <strong>{bookingRef.trim() ? bookingRef.trim() : "—"}</strong>
            </div>
        ...
        {mode === "confirm" ? (
          <div className="to-card" style={{ padding: 16, borderRadius: 14, marginTop: 14 }}>
            ...
            <div className="to-muted" style={{ marginTop: 6 }}>Status: <strong>{status}</strong></div>
            ...
            {feedback ? <div className="to-muted" style={{ marginTop: 10 }}>Feedback recorded: {feedback}</div> : null}
          </div>
        ) : null}
      </section>
    </main>
